...
func (b *Backend) LockVMOps(ctx context.Context, vmID string) (func(), error) {
	...
	// Lockless read: RunDir is immutable after create. Fail closed on a real read error (a missing record reads as the default path) — guessing the path could split the lock domain.
	if rec, ok, err := b.DB.Get(vmID); err != nil {
		return nil, fmt.Errorf("resolve run dir for %s: %w", vmID, err)
	} else if ok && rec.RunDir != "" {
		runDir = rec.RunDir
	}
