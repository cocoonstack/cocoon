...
# OUT_NAME picks the arch tag (gapps16-x86_64.tar default; gapps16-arm64.tar for an
# arm64-v8a Play system image). The extraction paths below are ABI-agnostic.
set -euo pipefail

OUT_NAME="${OUT_NAME:-gapps16-x86_64.tar}"
...
tar --numeric-owner --xattrs -C /tmp/gapps-out -cf "/out/$OUT_NAME" .
