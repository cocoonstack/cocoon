...
Docker, ReDroid 16, GMS/GApps, ADB, and one RFB/VNC endpoint. The directory name
is historical; the VM userspace is Ubuntu 22.04 (Jammy), selected for ReDroid
...
VM_IMAGE=local/ubuntu-redroid-gms:22.04-android16 \
bash build-cross.sh
```
...
```bash
cid="$(docker create local/ubuntu-redroid-gms:22.04-android16)"
