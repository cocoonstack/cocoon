...
- **Balloon**: 25% of memory auto-returned via virtio-balloon with deflate-on-OOM and free-page reporting (VMs with < 256 MiB memory skip balloon; `--no-balloon` opts a VM out at create, and clone/restore inherit it from the snapshot, which owns the restored device tree)
