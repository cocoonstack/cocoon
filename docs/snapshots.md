...
- **Keep the same base-image lineage.** Restore retains the target's base-image paths, boot configuration, and image references. `--force` does not make a different base image compatible: use `vm clone` for that. Cloud Hypervisor checks NIC MACs; Firecracker also requires the snapshot's recorded COW and data-disk paths to match the target. These checks do not establish base-image identity.
